...
const uniqueTeams = computed(() => new Set(submissions.value.map((s) => s.userId)).size)

const todayCount = computed(() => submissions.value.filter((s) => isSameDay(s.submittedAt)).length)

const videoTotalCount = computed(() => videos.value.length)

const videoTodayCount = computed(() => videos.value.filter((v) => isSameDay(v.submittedAt)).length)

const recentSubmissions = computed(() => submissions.value.slice(0, 3))
...
const videoStats = computed(() => [
  {
    label: 'Total Videos',
    value: videoTotalCount.value,
    accent: 'text-sky-400',
    loading: isVideoLoading.value,
  },
  {
    label: 'Videos Today',
    value: videoTodayCount.value,
...
<template>
  <main>
    ...
    <section class="mb-8 text-white">
      ...
      <h1 class="text-4xl md:text-5xl font-extrabold tracking-tight mt-2">Dashboard</h1>
      <p class="mt-3 max-w-2xl text-base md:text-lg leading-relaxed text-gray-400">
        Overview of repository and video submissions for ACLC CODEFEST 2026 Pre-Hacktoberfest
        Edition.
      </p>
    </section>
    ...
    >
      <div class="flex items-center justify-between gap-4 mb-6">
        <div>
          <p class="text-sm font-semibold uppercase tracking-wider text-purple-400">Latest</p>
          <h2 class="text-2xl md:text-3xl font-extrabold mt-1">Recent Repository Submissions</h2>
        </div>
        <RouterLink
          to="/judges/repository"
          class="shrink-0 px-4 py-2 rounded-xl text-sm font-bold transition bg-purple-500 hover:bg-purple-600"
        >
          View all
        </RouterLink>
      </div>
      ...
      <p v-else-if="!recentSubmissions.length" class="text-sm text-gray-400">
      ...
      <ul v-else class="flex flex-col gap-3">
        <li
          ...
          :key="s.id"
          class="p-4 rounded-2xl bg-white/5 border border-white/10 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4"
        >
          <div class="flex-1 min-w-0">
            ...
            <p v-if="s.groupName && s.projectName" class="text-xs text-gray-400 truncate">
              Project: {{ s.projectName }}
            </p>
            <p class="text-xs text-gray-400 truncate">
              Members: {{ displayMembers(s) || s.githubUsername || s.userId }} ·
